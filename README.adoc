...
IMPORTANT:  Backslashes worden opgevat als escape-karakters. Om backslashes in paden op te geven, escape deze met een extra backslash, bijvoorbeeld: `Importfiltermap=Submap1\\Submap2\\Submap3`. Je mag ze ook vervangen door een forward slash, bijvoorbeeld: 'Importfiltermap=Submap1/Submap2/Submap3'.

=== Parameters voor importscript
...
* `maakalleenlesgroepteams=waarde` : schakelaar voor aanmaken van een team voor alleen lesgroepen
...
* `samenvoegen=waarde` : bepaalt of uitvoer wordt toegevoegd aan de bestanden in de verzamelmap`

=== Verklaring van parameters

==== Schoolid = [tekenreeks]
Deze verplichte parameter bevat een unieke identificatie van de school. Je kunt hiervoor het BRIN van de school gebruiken of een andere unieke aanduiding. 

==== Schoolnaam = [tekenreeks]
Deze verplichte parameter bevat de weergavenaam van de school. 

==== MagisterUser = [tekenreeks]
Deze verplichte parameter bevat de gebruikersnaam voor toegang tot de Medius Webservice.

==== MagisterPassword = [tekenreeks]
Deze verplichte parameter bevat het wachtwoord voor toegang tot de Medius Webservice.

==== MagisterUrl = [tekenreeks]
Deze verplichte parameter bevat de URL voor toegang tot de Medius Webservice.

IMPORTANT: De parameters MagisterUser, MagisterPassword en MagisterUrl zijn vereist om toegang te krijgen tot de Medius Webservices. De LAS-beheerder maakt een gebruiker aan in de Webservice gebruikerslijst in Magister. De gebruikersnaam en wachtwoord moeten worden gegeven in `MagisterUser` en `MagisterPassword`. Deze gebruiker heeft toegangsrechten nodig tot de *_ADfuncties_* in de Medius Webservices. De MagisterUrl is de URL waar de webservices worden aangeboden. Dit bestaat uit de schoolspecifieke URL voor  Schoolwerkplek met daarachter de poort en het pad , welke meestal _:8800/doc_ is. De produktieomgeving is benaderbaar op poort 8800, de testomgeving op poort 8801 en de beta omgeving op poort 8802. De hele URL ziet er uit als `https://schooldomein.swp.nl:8800/doc`.  

==== Teamid_prefix = [tekenreeks]
...
==== maakalleenlesgroepteams = {'0'|'1'}
Geef '0' als je teams van klassen en lesgroepen wilt maken, '1' als je teams van alleen lesgroepen wilt maken. Bij verstek is de waarde '0'. 

==== Importfiltermap = [tekenreeks]
...
==== handhaafJPTMedewerkerCodeIsLogin = {'0'|'1'}
...
==== medewerker_id = {'code'|'loginaccount'|'ad_upn'|'csv_upn'}
...
==== leerling_id = { loginaccount'|'email'}
...
=== bon_match_docentlesgroep_aan_leerlingklas = {'0'|'1'}
...
=== docenten_per_team_limiet = [getal]
...
=== samenvoegen = {'0'|'1'} 
Deze parameter is een schakelaar met de waarde `0` of `1` die bepaalt of de uitvoer moet worden toegevoegd aan de bestanden in de verzamelmap. Door dit de waarde `0` te geven, is het mogelijk om de uitvoer in het huidige profiel te beoordelen, alvorens deze te uploaden naar School Data Sync. Wanneer de uitvoer naar wens is, kan door dit  waarde `1` te geven, de uitvoer worden samengevoegd met de bestanden in de verzamelmap om aan te bieden aan School Data Sync. 
...
TeamSync is met name bedoeld om zonder interactie als geplande taak worden uitgevoerd. 

Vanaf de PowerShell prompt gebruikmakend van verstekwaarden:
```
. .\Import-Magister.ps1
...
Met ingang van 'New School Data Sync' beschikbaar vanaf eind 2023, wordt de uitvoer van diverse TeamSync instanties samengevoegd in de verzamelmap om als één dataset te worden geupload naar School Data Sync. 

=== LET OP: Leerlingen, docenten en vakken moeten actief zijn in Magister

Probeer je TeamSync uit, maar krijg je geen actieve teams? Waarschijnlijk haal je de Magistergegevens op in de zomervakantie. Om een lijst met actieve teams te genereren, moeten de gegevens in Magister aan een aantal strikte voorwaarden voldoen:
...
Anno 2026 is School Data Sync in versie 2. De versie SDS wijkt in zoverre af van de oude School Data Sync dat het geen afzonderlijke synchronisatieprofielen ondersteunt. In de woorden van Microsoft heet het dat "Single Subscription - Multi-Academy Trust Support" vooralsnog ontbreekt. De consequentie hiervan is dat het gelijktijdig synchroniseren van verscheidene scholen naar School Data Sync alleen mogelijk is door de uitvoer van TeamSync voor verscheidene scholen/profielen samen te voegen in één bestandenverzameling voor School Data Sync. TeamSync ondersteunt dat met de variabele ```exportverzamelmap``` in de configuratie voor Export-SchoolDataSync.ps1.
