...
. Start Teamsync met het commando `teamsync.cmd`.
...
* leest instellingen uit configuratiebestand
...
=== Configuratiebestand
Dit bestand heeft bij verstek de naam `teamsync.ini`. Het bevat instellingen voor het TeamSync-script, zoals de gebruikersnaam, wachtwoord en URL voor het aanspreken van de webservice, de schoolnaam, het BRINnummer en een teamnaam-prefix. 
...
. Zie sectie "Identiteiten koppelen" voor een beschrijving van de koppelmethoden. 
...
=== Parameter Inifile 
Met de commandoregelparameter `-Inifile` wordt de bestandsnaam van een alternatief configuratiebestand opgegeven. De naam is geinterpreteerd als relatief ten opzichte van de map waarin het script staat. Deze voorziening maakt het mogelijk om gegevens van verscheidene instanties van Magister gescheiden te verwerken. 

Een voorbeeld : 

 <scriptnaam> -Inifile <bestandsnaam> 

waarbij `<bestandsnaam>` de naam aanduidt van een alternatief configuratiebestand. De commandoregelparameter heeft een aantal aliassen. Deze zijn `Inifilename`,`Inibestandsnaam`,`Config`,`Configfile`,`Configuratiebestand`. Een alternatief configuratiebestand kan dus ook worden opgegeven met bijvoorbeeld: 
...
* Het script `Transformeren-naar-SchoolDataSync.ps1` wordt één keer uitgevoerd voor elke school met een vschool-specifiek configuratiebestand: 
...
** `teamnaam_prefix` verschilt per school.
